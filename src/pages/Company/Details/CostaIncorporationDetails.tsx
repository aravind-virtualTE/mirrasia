...
import CrMemberDetail, { CrMemberData } from "@/components/shareholderDirector/CrMemberDetail";
import { getCrMemberData } from "@/components/ShrDirForm/cr/memberConfig";

type User = { _id: string; fullName?: string; email: string; role: string };

const COUNTRY_CODE = "CR";
const COUNTRY_LABEL = "Costa Rica";
...
/** ---------------- component ---------------- */
const CostaRicaCompanyDetails: React.FC<{ id: string }> = ({ id }) => {
  ...
  const [selectedMember, setSelectedMember] = useState<CrMemberData | null>(null);
  ...
  const showMemberDetails = async (member: any) => {
    try {
      if (!member.email) {
        toast({ description: "Member email not found", variant: "destructive" });
        return;
      }
      const data = await getCrMemberData(member.email);
      if (data) {
        setSelectedMember(data);
        setIsMemberDetailOpen(true);
      } else {
        toast({ description: "No detailed record found for this member" });
      }
    } catch (e) {
      toast({ description: "Failed to fetch member details", variant: "destructive" });
    }
  };
            ...
            <Card className="min-w-0">
              ...
              <CardContent className="grid gap-5 min-w-0">
                ...
                <div className="space-y-3 min-w-0">
                  ...
                  {((Array.isArray(form?.shareHolders) && form.shareHolders.length > 0) || (Array.isArray(form?.parties) && form.parties.length > 0)) ? (
                    <div className="rounded-md border min-w-0 overflow-hidden">
                      <div className="w-full max-w-full overflow-x-auto overflow-y-hidden">
                        <Table className="w-max min-w-full">
                          ...
                          <TableBody>
                            {(form.shareHolders || form.parties || []).map((p: any, index: number) => (
                              <TableRow
                                key={p._id || index}
                                className={`align-top ${!isEditing && p?.email ? "cursor-pointer" : ""}`}
                                onClick={!isEditing && p?.email ? () => showMemberDetails(p) : undefined}
                              >
...
export default CostaRicaCompanyDetails;
