...
//                 toast({ description: t('Company incorporation request submitted successfully!') });
//                 navigate('/dashboard')
//                 resetState()
//               } else {
...
//                             }`}>
//                             <h2 className="text-lg font-semibold mb-2">{t('AmlCdd.legal_assessment_title')}</h2>
//                         </aside>
//                         <div className="w-3/4 ml-4">
//                         <p className={`text-sm mb-4 ${theme === 'light' ? 'text-gray-800' : 'text-gray-200'}`}>
...
//                             <p className="text-sm text-gray-600">{t('AmlCdd.business_information_description')}</p>
...
//                                         <Label htmlFor={`${q.id}-yes`} className="text-sm">{t('AmlCdd.options.yes')}</Label>
//                                         </div>
//                                         <div className="flex items-center space-x-2">
//                                         <RadioGroupItem value="no" id={`${q.id}-no`} />
//                                         <Label htmlFor={`${q.id}-no`} className="text-sm">{t('AmlCdd.options.no')}</Label>  <Label htmlFor={`${q.id}-no`} className="text-sm">No</Label>
//                                         </div>
//                                         <div className="flex items-center space-x-2">
//                                         <RadioGroupItem value="unknown" id={`${q.id}-unknown`} />
//                                         <Label htmlFor={`${q.id}-unknown`} className="text-sm">{t('AmlCdd.options.unknown')}</Label>
//                                         </div>
//                                     </RadioGroup>
//                                 </div>
//                             ))}
...
//                 <DialogContent>
//                     <DialogHeader>
//                     <DialogTitle className="font-medium text-gray-700">{t('AmlCdd.dialog_acknowledgement_title')}</DialogTitle>
//                     </DialogHeader>
//                     <p className="text-sm text-gray-500">
//                     {t('AmlCdd.dialog_acknowledgement_message')}
//                     </p>
//                     <Button onClick={() => setInitialDialogOpen(false)}>{t('AmlCdd.dialog_button')}</Button>
//                 </DialogContent>
//             </Dialog>
    ...
    <>
        ...
        <CardContent>
          <div className="flex w-full p-4">
            ...
            <div className="w-3/4 ml-4">
              ...
              <RadioGroup value={businessInfoHkCompany.legal_assessment} onValueChange={(value) => handleQuestionChange('legal_assessment', value)} disabled={disabledQuestions['legal_assessment']}>
                {options.map((option) => (
                  <div key={option.value} className="flex items-center space-x-2">
                    <RadioGroupItem value={option.value} id={option.value} />
          ...
          <div className="flex w-full p-4">
            <aside className={`w-1/4 p-4 rounded-md shadow-sm ${theme === 'light' ? 'bg-blue-50 text-gray-800' : 'bg-gray-800 text-gray-200'}`}>
              <h2 className="text-lg font-semibold mb-2">{t('AmlCdd.business_information_title')}</h2>
              <p className="text-sm text-gray-600">{t('AmlCdd.business_information_description')}</p>
            </aside>
            <div className="w-3/4 ml-4">
              {questions.map((q) => (
                <div key={q.id} className="space-y-4 py-4">
                  ...
                  <RadioGroup value={businessInfoHkCompany[q.id]} onValueChange={(value) => handleQuestionChange(q.id, value)} disabled={disabledQuestions[q.id]}>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="yes" id={`${q.id}-yes`} />
                      <Label htmlFor={`${q.id}-yes`} className="text-sm">{t('AmlCdd.options.yes')}</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="no" id={`${q.id}-no`} />
                      <Label htmlFor={`${q.id}-no`} className="text-sm">{t('AmlCdd.options.no')}</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="unknown" id={`${q.id}-unknown`} />
                      <Label htmlFor={`${q.id}-unknown`} className="text-sm">{t('AmlCdd.options.unknown')}</Label>
      ...
      <Dialog open={secondDialogOpen} onOpenChange={setSecondDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
          <form onSubmit={handleSubmitSecondDialouge} className="space-y-6">
            <Card className="flex flex-row items-start space-x-3 rounded-md border p-2 shadow">
              <input type="checkbox" checked={acknowledgement} onChange={(e) => setAcknowledgement(e.target.checked)} className="form-checkbox my-4 cursor-pointer" />
              <div className="space-y-1 leading-none">
                <DialogTitle className="font-medium text-gray-700">{t('AmlCdd.dialog_acknowledgement_title')}</DialogTitle>
                <p className="text-sm text-gray-500">{t('AmlCdd.dialog_acknowledgement_message')}</p>
              </div>
            </Card>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto px-6">
            <p className="text-base text-gray-600 mb-6">{t('AmlCdd.dialog_consultation_message')}</p>
          </div>
          <div className="border-t px-6 py-4 mt-auto flex justify-center">
