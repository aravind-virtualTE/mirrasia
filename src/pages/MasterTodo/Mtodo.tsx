...
import { Layers, Loader2, PlusCircle, Unplug } from "lucide-react";
...
const ToDoList = () => {
  ...
  const [googleIntegration, setGoogleIntegration] = useAtom(googleIntegrationAtom);
  ...
  useEffect(() => {
    const bootstrap = async () => {
      ...
      // Fetch Google integration status for Google users
      if (isGoogleUser) {
        try {
          const status = await getGoogleIntegrationStatus();
          setGoogleIntegration({ ...status, loading: false });
        } catch {
          setGoogleIntegration({ connected: false, scopes: [], loading: false });
        }
      } else {
        setGoogleIntegration({ connected: false, scopes: [], loading: false });
      }
  ...
  const handleGoogleDisconnect = async () => {
    try {
      ...
      setGoogleIntegration({ connected: false, scopes: [], loading: false });
      toast({ title: "Google integration disconnected" });
    } catch {
      ...
      <div className="flex flex-col gap-3 mb-4">
        ...
        <div className="flex flex-wrap justify-between items-center gap-3">
          ...
          <div className="flex items-center gap-4">
            ...
            {isGoogleUser && !googleIntegration.loading && (
              googleIntegration.connected ? (
                <Button variant="outline" onClick={handleGoogleDisconnect} className="h-8 px-3 text-xs text-green-700 border-green-300">
                  <Unplug className="mr-2 h-4 w-4" />
                  Google Connected
                </Button>
              ) : (
                <Button variant="outline" onClick={handleGoogleConnect} className="h-8 px-3 text-xs">
