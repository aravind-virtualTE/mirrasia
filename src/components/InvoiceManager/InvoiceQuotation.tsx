...
export default function QuoteBuilder({
  ...
  const expiryDate = plusDaysISO(quoteValidDays);
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="text-sm text-muted-foreground">
                  Issued on: {issueDate} • Expiry: {expiryDate}
                </div>
              </div>
              <div className="text-right">
                <div className="font-semibold">{companyName}</div>
                <div className="text-sm text-muted-foreground max-w-[36rem]">{companyAddress}</div>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            ...
            <div className="rounded-lg border p-4">
              ...
              <div className="grid gap-1 text-sm">
                ...
                <div><span className="text-muted-foreground">Interested Country: </span>{client.interestedCountry || "-"}</div>
              </div>
            </div>
              ...
              <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                {selectedCountries.length > 0 && (
                  <div className="rounded-lg border p-4">
                    ...
                    <ul className="text-sm list-disc pl-5 space-y-1">
                      {selectedCountries.map((c) => <li key={c.title}>{c.title}</li>)}
                    </ul>
                  </div>
                )}
                {selectedIndustries.length > 0 && (
                  <div className="rounded-lg border p-4">
                    ...
                    <ul className="text-sm list-disc pl-5 space-y-1">
                      {selectedIndustries.map((i) => <li key={i.title}>{i.title}</li>)}
                    </ul>
                  </div>
                )}
              </div>
            ...
            <div className="rounded-lg border overflow-hidden">
              <div className="px-4 py-3 font-medium bg-muted/40">Product or Service</div>
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {selectedBillables.length === 0 ? (
                    ...
                    selectedBillables.map((it) => {
                      const unit = parseFloat(it.price || "0") || 0;
                      return (
